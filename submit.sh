...
# /tmp_folders/scratch/hector/newcropspatch/build_jura/guess
# as a parallel job on a multicore system.
#
# Created automatically on fre 16 apr 2021 08:47:07 CEST
# 
# Usage: 
#
...
do
  ...
  cd run$a
  ( if ! /tmp_folders/scratch/hector/newcropspatch/build_jura/guess -input $INPUT_MODULE $INSFILE 1>> ../stdout.txt 2>> ../stderr.txt ; then
      echo -n $a" " >> ../failed.txt
  fi ) &
  cd ..
done
